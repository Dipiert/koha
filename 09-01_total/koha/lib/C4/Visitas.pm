...
BEGIN {
    ...
    push @EXPORT, qw(PutGrupalDetallada);
...
sub PutUsuario {    
	...
	my $categoria = "GE";  #Esto hay que cambiar si o si, es horrible
	my $branchcode = "bpm";
	my $afiliado = "N";
	my $borrowernumber;
...
sub PutGrupalDetallada{
	my ($idvisita, $dni, $localidad, $direccion) = @_;
	my $dbh   = C4::Context->dbh;
	my $query = 'INSERT INTO grupalDetallada(idvisita,dni,localidad,direccion) VALUES '."($idvisita,'$dni','$localidad','$direccion')";
    my $sth = $dbh->prepare($query);
    $sth->execute();	
}
