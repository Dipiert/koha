...
<xsl:if test="$DisplayOPACiconsXSLT!='0'">
 ...
 <xsl:choose>
 <xsl:when test="$typeOf008='Mon'"><img src="/opac-tmpl/lib/famfamfam/BK.png" alt="Libro" title="Libro" /> Libro</xsl:when>
 <xsl:when test="$typeOf008='Per'"><img alt="Periódica" src="/opac-tmpl/lib/famfamfam/AR.png" title="Periódica" /> Periódica</xsl:when>
...
<!-- Links found in 856$u. This is copied verbatim from MARC21 detail view and should be identical to code in NORMARC detail view (except for $Show856uAsImage='Results'). -->
 ...
 <xsl:otherwise>
 <xsl:text>Haga clic aquí para acceso en línea</xsl:text>
 </xsl:otherwise>
 </xsl:choose>
 </a>
 <xsl:choose>
...
<span class="results_summary">
 ...
 <xsl:choose>
 <xsl:when test="count(key('item-by-status', 'available'))=0 and count(key('item-by-status', 'reference'))=0">No hay ítems disponibles </xsl:when>
